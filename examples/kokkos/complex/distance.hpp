...
template <typename T>
T epsilonForExponent(int exponent) {
...
template <typename T>
T distanceTowardsExponentUpperBound(T value, int exponent) {
    return (std::exp2(T(exponent + 1)) - value) / epsilonForExponent<T>(exponent);
}

template <typename T>
T distanceTowardsExponentLowerBound(T value, int exponent) {
    return (value - std::exp2(T(exponent))) / epsilonForExponent<T>(exponent);
}

template <typename T>
T distance(T from, T to) {
    using Traits = traits::FloatingPoint<T>;
    ...
    if (exponentFrom == exponentTo) {
        ...
        if (exponentTo - exponentFrom > 1) {
            ...
            if (exponentFrom + 1 >= Traits::smallestNormalNumberExponent) {
                out += Traits::amountOfNormalNumbersPerExponent * (exponentTo - exponentFrom - 1);
            }
            // Do every in-between exponents represent subnormal numbers?
                ...
                while (exponent < exponentTo) {
                    out += std::exp2(T(exponent - Traits::smallestSubnormalNumberExponent));
                ...
                out += Traits::amountOfNormalNumbersPerExponent * (exponentTo - exponent);
            }
        }
        ...
        // for large numbers.
        out +=
            (distanceTowardsExponentUpperBound(from, exponentFrom) + distanceTowardsExponentLowerBound(to, exponentTo));
    }
...
T distance(T from, T to) {
    ...
    if (std::signbit(from) != std::signbit(to)) {
        ...
        return 2 + (from == -smallestSubnormal ? 0 : internal::distance(smallestSubnormal, -from))
             + (to == smallestSubnormal ? 0 : internal::distance(smallestSubnormal, to));
    }
