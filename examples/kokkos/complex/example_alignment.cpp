...
struct Complex {
    ...
    KOKKOS_FUNCTION constexpr Complex& operator+=(const Complex& other) noexcept {
        real_ += other.real_;
        imag_ += other.imag_;
        return *this;
    }

    KOKKOS_FUNCTION friend constexpr Complex operator*(const Complex& src_a, const Complex& src_b) noexcept {
        return {
   ...
   public:
    static void run(const Kokkos::Cuda& exec) {
        ...
        const Kokkos::Profiling::ScopedRegion region(
            std::alignment_of_v<complex_t> == sizeof(typename complex_t::value_type) ? "default" : "specified");
