   ...
   public:
    static void run(const Kokkos::Cuda& exec) {
        ...
        Kokkos::parallel_for(
            ...
            FunctorMax<Method::CUDA_HMAX, view_t>{.dst = dst, .src_a = src_a, .src_b = src_b});
        Kokkos::parallel_for(
            Kokkos::RangePolicy(exec, 0, size),
            FunctorMax<Method::FMAX, view_t>{.dst = dst, .src_a = src_a, .src_b = src_b});
        Kokkos::parallel_for(
            Kokkos::RangePolicy(exec, 0, size),
            FunctorMax<Method::KOKKOS_FMAX, view_t>{.dst = dst, .src_a = src_a, .src_b = src_b});
