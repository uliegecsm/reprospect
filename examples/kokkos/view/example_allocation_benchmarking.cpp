...
struct MemoryPool {
    ...
    MemoryPoolState state() const {
        ...
        KOKKOS_IMPL_CUDA_SAFE_CALL(
            cudaMemPoolGetAttribute(ptr, cudaMemPoolAttrReservedMemCurrent, &state.reserved_mem_current));
        KOKKOS_IMPL_CUDA_SAFE_CALL(
            cudaMemPoolGetAttribute(ptr, cudaMemPoolAttrReservedMemHigh, &state.reserved_mem_high));
        KOKKOS_IMPL_CUDA_SAFE_CALL(
            cudaMemPoolGetAttribute(ptr, cudaMemPoolAttrUsedMemCurrent, &state.used_mem_current));
        KOKKOS_IMPL_CUDA_SAFE_CALL(cudaMemPoolGetAttribute(ptr, cudaMemPoolAttrUsedMemHigh, &state.used_mem_high));
        KOKKOS_IMPL_CUDA_SAFE_CALL(
            cudaMemPoolGetAttribute(ptr, cudaMemPoolAttrReleaseThreshold, &state.release_threshold));
...
class WithCUDA : public Common {
   ...
   public:
    ...
    {
        KOKKOS_IMPL_CUDA_SAFE_CALL(cudaMallocAsync((void**) ptr, size * sizeof(scalar_t), exec->cuda_stream()));
...
void parameters(::benchmark::internal::Benchmark* benchmark) {
    ...
    benchmark
        ...
        ->Args({1, threshold - 1000})
        ->Args({1, threshold + 1000})
        ->Args({4, threshold - 1000})
        ->Args({4, threshold + 1000})
        ->Args({8, threshold - 1000})
        ->Args({8, threshold + 1000})
        ->Args({12, threshold - 1000})
        ->Args({12, threshold + 1000})
        ->ArgsProduct({{4, 8, 12}, ::benchmark::CreateRange(128, 128 << 17, 4)})
        ->ArgsProduct({{1}, ::benchmark::CreateRange(128, 128 << 19, 4)});
}
