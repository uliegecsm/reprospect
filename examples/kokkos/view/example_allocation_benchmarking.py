...
class TestAllocation(CMakeAwareTestCase):
    ...
    def test_memory_pool_attributes(self, raw) -> None:
        ...
        assert attributes['ReservedMemCurrent'].nunique(dropna=False) == 1 # ruff:ignore[pandas-nunique-constant-series-check]
        assert attributes[    'UsedMemCurrent'].nunique(dropna=False) == 1 # ruff:ignore[pandas-nunique-constant-series-check]
