...
__global__ void global_kernel(
    ...
    const T* __restrict__ const src_a,
    const T* __restrict__ const src_b,
...
class Restrict {
   ...
   public:
    ...
    void restrict_recast_lambda(const Kokkos::Cuda& exec) const {
        Kokkos::parallel_for(
            "restrict_recast_lambda",
            Kokkos::RangePolicy(exec, 0, size),
            FunctorRestrictRecastLambda<scalar_t>{.src_a = src_a.data(), .src_b = src_b.data(), .dst = dst.data()});
    }

    void restrict_recast_local(const Kokkos::Cuda& exec) const {
        Kokkos::parallel_for(
            "restrict_recast_local",
            Kokkos::RangePolicy(exec, 0, size),
            FunctorRestrictRecastLocal<scalar_t>{.src_a = src_a.data(), .src_b = src_b.data(), .dst = dst.data()});
    }
    ...
    void restrict_member(const Kokkos::Cuda& exec) const {
        Kokkos::parallel_for(
            "restrict_member",
            Kokkos::RangePolicy(exec, 0, size),
            FunctorRestrictMember<scalar_t>{.src_a = src_a.data(), .src_b = src_b.data(), .dst = dst.data()});
    }

    void restrict_view_memory_trait(const Kokkos::Cuda& exec) const {
        Kokkos::parallel_for(
            "restrict_view_memory_trait",
            Kokkos::RangePolicy(exec, 0, size),
...
#define RUN_ONE_CASE(exec, case)                                                                                       \
    tester.reset(exec);                                                                                                \
    tester.case (exec); tester.check(exec);
