"""
Thin user-facing factories for the matchers in :py:mod:`reprospect.testing.sass.composite_impl`.
"""
...
def instruction_count_is(matcher: instruction.InstructionMatcher, count: int) -> composite_impl.CountInSequenceMatcher:
    ...
    >>> from reprospect.testing.sass.composite import instruction_count_is
    >>> from reprospect.testing.sass.instruction import Fp32AddMatcher
...
def instructions_are(*matchers: instruction.InstructionMatcher | composite_impl.SequenceMatcher) -> composite_impl.OrderedInSequenceMatcher:
    ...
    >>> from reprospect.testing.sass.composite   import instructions_are
...
def unordered_instructions_are(*matchers: instruction.InstructionMatcher | composite_impl.SequenceMatcher) -> composite_impl.UnorderedInSequenceMatcher:
    ...
    >>> from reprospect.testing.sass.composite   import unordered_instructions_are
    >>> from reprospect.testing.sass.instruction import OpcodeModsMatcher
...
def instructions_contain(matcher: instruction.InstructionMatcher | composite_impl.SequenceMatcher) -> composite_impl.InSequenceMatcher:
    ...
    >>> from reprospect.testing.sass.composite   import instructions_are, instructions_contain
    >>> from reprospect.testing.sass.instruction import OpcodeModsMatcher
...
def any_of(*matchers: instruction.InstructionMatcher | composite_impl.SequenceMatcher) -> composite_impl.AnyOfMatcher:
    ...
    >>> from reprospect.testing.sass.composite   import any_of
    >>> from reprospect.testing.sass.instruction import OpcodeModsMatcher
    >>> matcher = any_of(
    ...     OpcodeModsMatcher(opcode = 'YIELD', operands = False),
    ...     OpcodeModsMatcher(opcode = 'NOP', operands = False),
...
def interleaved_instructions_are(*matchers: instruction.InstructionMatcher | composite_impl.SequenceMatcher) -> composite_impl.OrderedInterleavedInSequenceMatcher:
    ...
    >>> from reprospect.testing.sass.composite import interleaved_instructions_are
    >>> from reprospect.testing.sass.instruction import OpcodeModsMatcher
