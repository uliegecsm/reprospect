...
int main() {
...

    {
        ...
        REPROSPECT_CHECK_CUDART_CALL(
            cudaMemcpyAsync(vec_x, vec_x_h.data(), size * sizeof(float), cudaMemcpyHostToDevice, stream_A));
        REPROSPECT_CHECK_CUDART_CALL(
            cudaMemcpyAsync(vec_y, vec_y_h.data(), size * sizeof(float), cudaMemcpyHostToDevice, stream_B));
    ...
    REPROSPECT_CHECK_CUDART_CALL(
        cudaMemcpyAsync(result.data(), vec_y, size * sizeof(float), cudaMemcpyDeviceToHost, stream_B));
