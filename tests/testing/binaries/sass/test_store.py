...
class TestStoreMatcher:
    """
    Tests for :py:class:`reprospect.testing.binaries.sass.instruction.store.StoreMatcher`
    and :py:class:`reprospect.testing.binaries.sass.instruction.store.StoreGlobalMatcher`.
    """
    def test(self) -> None:
    ...
    def test_elementwise_add_restrict(self, request, workdir, parameters: Parameters, cmake_file_api: cmake.FileAPI) -> None:
        """
        Test store with :py:const:`tests.testing.binaries.sass.test_instruction.CODE_ELEMENTWISE_ADD_RESTRICT`.
        """
