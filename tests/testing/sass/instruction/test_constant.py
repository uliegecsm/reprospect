...
from reprospect.testing.sass.instruction.constant import (
    ConstantMatch,
    ConstantMatcher,
    MathModifier,
)
...
class TestConstantMatcher:
    """
    Tests for :py:class:`reprospect.testing.sass.instruction.constant.ConstantMatcher`.
    """
    ...
    def test_constant_neg(self) -> None:
        ...
        assert ConstantMatcher(bank='0x0'                       ).match(CONSTANT) == self.CONSTANTS[CONSTANT]
        assert ConstantMatcher(bank='0x0', math=MathModifier.NEG).match(CONSTANT) == self.CONSTANTS[CONSTANT]
        assert ConstantMatcher(bank='0x0', math=MathModifier.INV).match(CONSTANT) is None
