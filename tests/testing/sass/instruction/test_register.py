...
from reprospect.testing.sass.instruction.register import (
    MathModifier,
    Register,
    RegisterMatch,
    RegisterMatcher,
...
class TestRegister:
    """
    Tests for :py:class:`reprospect.testing.sass.instruction.register.Register`.
    """
    def test_reg_vs_ureg(self) -> None:
        """
        Ensure that :py:attr:`reprospect.testing.sass.instruction.register.Register.REG` does not
        match what :py:attr:`reprospect.testing.sass.instruction.register.Register.UREG` matches
        (and vice versa).
        """
...
class TestRegisterMatcher:
    """
    Tests for :py:class:`reprospect.testing.sass.instruction.register.RegisterMatcher`.
    """
    REGISTERS: typing.Final[dict[str, RegisterMatch]] = {
        ...
        '-|R42|':    RegisterMatch(rtype=RegisterType.GPR,   index=42, reuse=False, math=MathModifier.NEG_ABS),
