...
from reprospect.testing.sass.composite import instructions_contain
...
class TestConvertIntToFp:
    """
    Tests for :py:class:`reprospect.testing.sass.matchers.convert_int_to_fp.ConvertIntToFp`.
    """
...
"""
    ...
    @pytest.mark.parametrize(
        ('src_c_type', 'src_dtype', 'dst_c_type', 'dst_dtype'), [
            ('int', numpy.int32, 'double', numpy.float64),
            ('int', numpy.int32, 'float', numpy.float32),
            ('long long', numpy.int64, 'double', numpy.float64),
            ('long long', numpy.int64, 'float', numpy.float32),
            ('unsigned int', numpy.uint32, 'double', numpy.float64),
            ('unsigned int', numpy.uint32, 'float', numpy.float32),
            ('unsigned long long', numpy.uint64, 'double', numpy.float64),
            ('unsigned long long', numpy.uint64, 'float', numpy.float32),
    ], ids=str)
    def test(self, request, src_c_type: str, src_dtype: ConvertibleTypeInfo, dst_c_type: str, dst_dtype: ConvertibleTypeInfo, parameters: Parameters, workdir: pathlib.Path, cmake_file_api) -> None:
        ...
        FILE.write_text(self.CODE.format(fp_type=dst_c_type, int_type=src_c_type))

        decoder, _ = get_decoder(cwd=workdir, arch=parameters.arch, file=FILE, cmake_file_api=cmake_file_api)

        instructions_contain(ConvertIntToFp(arch=parameters.arch, src_dtype=src_dtype, dst_dtype=dst_dtype)).assert_matches(decoder.instructions)
