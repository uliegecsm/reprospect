...
class TestAtomicMatcher:
    """
    Tests for :py:class:`reprospect.testing.sass.instruction.atomic.AtomicMatcher`.
    """
...
"""
    ...
    def get_atomicCAS_thread_scope(*, size: int, arch: NVIDIAArch, cuda_compiler: CMakeToolchainCompiler) -> ThreadScope:
        """
        Get the expected :py:data:`reprospect.testing.sass.instruction.ThreadScope` for :code:`atomicCAS`.
        """
    ...
    def assert_atomicCAS_ptx(*, output: pathlib.Path, matcher: AtomicMatcher, cuda_compiler: CMakeToolchainCompiler) -> None:
        """
        Check for the expected PTX code in `output` given the :py:data:`reprospect.testing.sass.instruction.ThreadScope`.

        According to:

        * https://docs.nvidia.com/cuda/cuda-programming-guide/05-appendices/cpp-language-extensions.html#legacy-atomic-functions

        it should generate :py:data:`reprospect.testing.sass.instruction.ThreadScope.DEVICE` instructions. However, as noted in:
        ...
        * https://forums.developer.nvidia.com/t/atomiccas-cuda-13-2-unexpectedly-generates-system-scope-atomic-instruction-starting-from-cc100/365449
        ...
        it may unexpectedly generate :py:data:`reprospect.testing.sass.instruction.ThreadScope.SYSTEM` instructions.
    ...
    def test_min_relaxed_device_int(self, request, workdir, parameters: Parameters, cmake_file_api: cmake.FileAPI):
        """
        Test with :py:attr:`CODE_MIN` for `int` and :py:data:`reprospect.testing.sass.instruction.ThreadScope.DEVICE` scope.
        """
        FILE = workdir / f'{request.node.originalname}.{parameters.arch.as_sm}.cu'
    ...
    def test_min_relaxed_system_int(self, request, workdir, parameters: Parameters, cmake_file_api: cmake.FileAPI):
        """
        Test with :py:attr:`CODE_MIN` for `int` and :py:data:`reprospect.testing.sass.instruction.ThreadScope.SYSTEM` scope.
        """
        FILE = workdir / f'{request.node.originalname}.{parameters.arch.as_sm}.cu'
    ...
    def test_min_relaxed_device_long_long_int(self, request, workdir, parameters: Parameters, cmake_file_api: cmake.FileAPI):
        """
        Test with :py:attr:`CODE_MIN` for `long long int` and :py:data:`reprospect.testing.sass.instruction.ThreadScope.DEVICE` scope.
        """
        FILE = workdir / f'{request.node.originalname}.{parameters.arch.as_sm}.cu'
    ...
    def test_min_relaxed_device_unsigned_long_long_int(self, request, workdir, parameters: Parameters, cmake_file_api: cmake.FileAPI):
        """
        Test with :py:attr:`CODE_MIN` for `unsigned long long int` and :py:data:`reprospect.testing.sass.instruction.ThreadScope.DEVICE` scope.
        """
        FILE = workdir / f'{request.node.originalname}.{parameters.arch.as_sm}.cu'
    ...
    def test_compare_exchange_system(self, request, consistency: str, workdir, parameters: Parameters, cmake_file_api: cmake.FileAPI):
        """
        Test with :py:attr:`CODE_COMPARE_EXCHANGE` for `unsigned long long int`, :py:data:`reprospect.testing.sass.instruction.ThreadScope.SYSTEM` scope.
